...
The quality of feature selection is assessed by Cohen's kappa \cite[]{cohen_coefficient_1960}. This kappa is measures similarity between two sets of categorical values. One set contains the feature selection indicators: $s(j) := \mathbbm{1}\{\hat{\beta}_j \neq 0\}$, while the other set contains the true non-zero feature indicators: $t(j) := \mathbbm{1}\{\beta_j \neq 0\}$. We calculate Cohen's $\kappa$ as:
  ...
  this using the data introduced in Section [! section ref here] of the Main 
  Document, randomly dividing 
  the features into three groups. We fixed the group sizes to the group sizes 
  used in the three-group setting in Section [! section ref here] of the Main
  Docuemtn. Under this random 
  ...
  repeated the procedure 100 times and present the results in [! figure ref]. 
  From this figure we see that the estimates for \texttt{gren} are close to one.
  The estimates by \texttt{GRridge} show much more variation. Additionally, 
  \texttt{GRridge} gives slightly biased estimates: the median penalty parameter
  estimates are [! medians here] for the three groups. In contrast, the median 
  estimates of \texttt{gren} are [! medians here].
