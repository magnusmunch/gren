...
<<<<<<< HEAD
  ...
  \ref{md-sec:colorectal}--\ref{md-sec:lymphnode}. We
...
=======
  \ref{sec:variationaldistributions}. In Section \ref{sec:updatingequations}
  the variational and empirical Bayes updating equations for the algorithm are
  given. Section
  \ref{sec:computationalcomplexity} shows how we may reduce the computational
  complexity of our variational Bayes procedure. Extensions of our
  method are
  ...
  apply our method to a study on metabolomics in Alzheimer's in Section
  \ref{sec:alzheimer} and to study in microRNAs in cervical cancer data 
  in Section \ref{sec:cervical}. The computational times
  of the method are explored in Section \ref{sec:timings}. Finally, 
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
=======
  Let $\mathbf{c} = \begin{bmatrix} c_1 & \cdots & c_n \end{bmatrix} \tr$. Furthermore, 
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
=======
      \E [\log q(\bomega)] & =  \m \tr \log [1 + \exp(\mathbf{c})] - n \log 2 - 
      \frac{1}{2} \m \tr \mathbf{c} - \sum_{i=1}^n \log \Gamma (m_i) - \frac{1}{2} 
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
=======
	    \tr \X \bmu^{(t)} + \m \tr \left\{ \frac{1}{2} \mathbf{c}^{(t)} - 
	    \log [1 + \exp(\mathbf{c}^{(t)})] \right\} \label{eq:elboomega4}\\
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
<<<<<<< HEAD
  Inspection of the updating equations in the MD, learns us that the 
=======
  Inspection of the updating equations (\ref{eq:VBupdateequations}), 
  learns us that the 
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
=======
    \m \tr \{ \frac{1}{2} \mathbf{c}^{(t)} - \log [ \exp(\mathbf{c}^{(t)}) + 
    1 ]\} +
    \frac{1}{2}
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
=======
  ...
  \section{Updating equations}\label{sec:updatingequations}
  If we fill in the expectations and variances in the variational posterior
  from Section \ref{sec:variationaldistributions} and maximise with respect
  to the variational parameters, we arrive at the following updating equations:
  \begin{subequations}\label{eq:VBupdateequations}
		...
		\end{align}
	\end{subequations}
	which we iterate until convergence. Here, $\bLambda'$ is a diagonal matrix 
	with entries 
	...
	element of a column vector, respectively. 
	
	To obtain the empirical Bayes updating equations, we first derive the 
	expected log likelihood under the variational posterior as:
  \begin{align*}
  ...
  \end{align*}
  Here, $\propto$ denotes proportionality with respect to $\blambda'$. The 
  resulting updating equations for the hyperparameters are now:
...
<<<<<<< HEAD
  ...
  in the MD, reveals
=======
  inspection of the updating equations in (\ref{eq:VBupdateequations}), reveals
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
=======
  Now we rewrite the calculation of $\mathbf{c} = \begin{bmatrix} c_1 & 
  \cdots & c_n
  \end{bmatrix} \tr$ as:
  \begin{align*}
    \mathbf{c} & = \begin{bmatrix} \sqrt{\x_1 \bSigma \x_1 + 
    (\x_1 \tr \bmu)^2} & 
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
=======
  We simulated data according to five different scenarios that are explained
  in more detail below. For the regular elastic net, \texttt{gren},
  \texttt{cMCP}, 
	...
	$\alpha=1$. In \texttt{cMCP} and \texttt{gel}, $\alpha$
	...
	from a $p$-dimensional zero-centred Gaussian distribution, where $p$ varies 
	from $p=900$ to $p=1000$. The covariance matrix of the features and 
	simulation of the model parameters $\beta_j$, $j=1, \dots, p$ varies between
  Scenarios. We 
	simulated the outcomes $y_i$, $i=1, \dots, n$, from a binary logistic model: 
	$y_i \sim \mathcal{B}(1, \expit (\x_i \tr \bbeta))$. 
	To mitigate the influence
	of random variation, we repeated each scenario 100 times 
	and report average area under the receiver operator curve (AUC) and 
	Brier skill score in Figures \ref{fig:lines_simulations_res1_auc_briers}-
	\ref{fig:lines_simulations_res5_auc_briers}. AUC and Brier skill score are 
	measures of discrimination and calibration, respectively, and for both a 
	higher value indicates better predictive performance.
	For the methods with an $\alpha$ parameter, we display the best-performing 
	$\alpha$. Results for the other $\alpha$'s are presented in 
	Figures \ref{fig:lines2_simulations_res1_auc_briers}-
	\ref{fig:lines2_simulations_res5_auc_briers}-
	
<<lines_simulations_res1_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (i).", out.width="100%", fig.asp=1/2>>=
@

<<lines_simulations_res2_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (ii).", out.width="100%", fig.asp=1/2>>=
@

<<lines_simulations_res3_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (iii).", out.width="100%", fig.asp=1/2>>=
@

<<lines_simulations_res4_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (iv).", out.width="100%", fig.asp=1/2>>=
@

<<lines_simulations_res5_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (v).", out.width="100%", fig.asp=1/2>>=
@
...
=======
	...
	Results for different values of the tuning parameter $\alpha$ than presented
	in MD Section \ref{md-sec:simulations} are given in 
	Figures \ref{fig:lines2_simulations_res1_auc_briers}-
	\ref{fig:lines2_simulations_res5_auc_briers}. Overall, these results follow 
	the same pattern as the ones presented in MD Section \ref{md-sec:simulations}.
...
@
  ...
  To assess the effects of model misspecification we conducted a series of 
  simulations under different modes of misspecification and simulation settings.
  We investigate model misspecification by comparing \texttt{gren} to the
  regular elastic net.
  To investigate the effect of misspecifying the link function, we simulated
  according to Scenarios 4 and 5, where we changed the response simulation:
	$y_i \sim \mathcal{B}(1, [\exp (\x_i \tr \bbeta) + \kappa]/
	[\exp (\x_i \tr \bbeta) + 1 + \kappa])$ as in \cite{komori_asymmetric_2016}, 
	where the link function is unbalanced for $\kappa \neq 0$. We use 
	$\kappa \in \{ 0.1, 0.5, 1 \}$, which leads to 6 different simulations 
	from Scenarios 4 and 5 in total. We present the results in Figures
	\ref{fig:lines2_simulations_res6_auc_briers}-
	\ref{fig:lines2_simulations_res11_auc_briers}. From the Figures we see that 
	\texttt{gren} outperforms the regular elastic net even with a misspecified 
	link function in terms of AUC. For the Brier skill score there seems to be 
	some detrimental effect of the misspecification.

<<lines2_simulations_res6_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 4 for a misspecified link function with $\\kappa=0.1$.", out.width="100%", fig.asp=1/2>>=
@

<<lines2_simulations_res7_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 4 for a misspecified link function with $\\kappa=0.5$.", out.width="100%", fig.asp=1/2>>=
@

<<lines2_simulations_res8_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 4 for a misspecified link function with $\\kappa=1$.", out.width="100%", fig.asp=1/2>>=
@

<<lines2_simulations_res9_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 5 for a misspecified link function with $\\kappa=0.1$.", out.width="100%", fig.asp=1/2>>=
@

<<lines2_simulations_res10_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 5 for a misspecified link function with $\\kappa=0.5$.", out.width="100%", fig.asp=1/2>>=
@

<<lines2_simulations_res11_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 5 for a misspecified link function with $\\kappa=1$.", out.width="100%", fig.asp=1/2>>=
@

  In addition to misspecification of the response relation, model 
  misspecification may also occur in the feature relation. To investigate this
  we simulated data that includes non-linear relations between linear predictor
  and features. To do so, we considered the following link function for part of 
  the features: $\expit [\max(0,x_{ij})\beta_j]$. We apply this non-linear 
  relation in simulation Scenarios 4 and 5, where we consider three fractions
  of features for which we apply the non-linear relation: 0.2, 0.4, and 0.6, 
  evenly divided over the groups of features. 
  Meaning that 0.8, 0.6, and 0.4 of the features are linearly related to the
  link function, respectively. This leads again to 6 different simulation 
  settings: three in Scenario 4 and three in Scenario 5. We present the results
  in Figures \ref{fig:lines2_simulations_res12_auc_briers}-
  \ref{fig:lines2_simulations_res14_auc_briers}.
  From the Figures we see that \texttt{gren} still outperforms the regular 
  elastic net, albeit by a slightly smaller margin then with a correctly
  specified feature relation.
	
<<lines2_simulations_res12_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 4 for a non-linear feature relation with the fraction of non-linear relations 0.2.", out.width="100%", fig.asp=1/2>>=
@

<<lines2_simulations_res13_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 4 for a non-linear feature relation with the fraction of non-linear relations 0.4.", out.width="100%", fig.asp=1/2>>=
@

<<lines2_simulations_res14_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 4 for a non-linear feature relation with the fraction of non-linear relations 0.6.", out.width="100%", fig.asp=1/2>>=
@
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
...
<<<<<<< HEAD
<<lines_micrornaseq_colorectal_cancer_res1_auc, fig.cap="Estimated AUC for the colorectal cancer example.", out.width="100%", fig.asp=1>>=
@
  
<<lines_micrornaseq_colorectal_cancer_res1_briers, fig.cap="Estimated Brier skill score for the colorectal cancer example.", out.width="100%", fig.asp=1>>=
=======
...
<<<<<<< HEAD
  \section{Application to RNAseq in lymph node cancer}
  \label{sec:lymphnode}
  In addition to the results presented in Section \ref{md-sec:lymphnode} of the 
=======
...
<<<<<<< HEAD
  \ref{fig:lines_rnaseq_lymph_node_metastasis_res1_auc}--
  \ref{fig:lines_rnaseq_lymph_node_metastasis_res1_briers}
<<lines_rnaseq_lymph_node_metastasis_res1_auc, fig.cap="Estimated AUC for the lymph node cancer example.", out.width="100%", fig.asp=1>>=
@
  
<<lines_rnaseq_lymph_node_metastasis_res1_briers, fig.cap="Estimated Briers skill score for the lymph node cancer example.", out.width="100%", fig.asp=1>>=
=======
  \ref{fig:lines_rnaseq_oral_cancer_metastasis_res1_auc}--
  \ref{fig:lines_rnaseq_oral_cancer_metastasis_res1_briers}
<<lines_rnaseq_oral_cancer_metastasis_res1_auc, fig.cap="Estimated AUC for the oral cancer example with (a) $\\alpha=0.05$, (b) $\\alpha=0.5$, and (c) $\\alpha=0.95$.", out.width="100%", fig.asp=1>>=
@
  
<<lines_rnaseq_oral_cancer_metastasis_res1_briers, fig.cap="Estimated Briers skill score for the oral cancer example with (a) $\\alpha=0.05$, (b) $\\alpha=0.5$, and (c) $\\alpha=0.95$.", out.width="100%", fig.asp=1>>=
>>>>>>> 31db1da7fc892df40e05ce4287d82f38e942befc
@
...
@
  ...
  In this example, \texttt{GRridge} and \texttt{gren} underperform due to a 
  small number of features to learn the penalty parameters from. The other 
  methods estimate one global penalty parameter, so do not suffer from the 
  lower dimensionality of the problem.
=======
<<lines_metabolomics_alzheimer_res1_auc, fig.cap="Estimated AUC for the alzheimer example with with (a) $\\alpha=0.05$, (b) $\\alpha=0.5$, and (c) $\\alpha=0.95$.", out.width="100%", fig.asp=1>>=
@

<<lines_metabolomics_alzheimer_res1_briers, fig.cap="Estimated Brier skill score for the alzheimer example with (a) $\\alpha=0.05$, (b) $\\alpha=0.5$, and (c) $\\alpha=0.95$.", out.width="100%", fig.asp=1>>=
@
...
<<<<<<< HEAD
  sequencing analysis resulted in 2,576 transcripts. The data was normalized 
=======
...
<<<<<<< HEAD
=======
  \section{Computational times}\label{sec:timings}
<<>>=
bench1 <- read.table("results/micrornaseq_colorectal_cancer_bench1.csv", 
                     stringsAsFactors=FALSE)

bench2 <- read.table("results/rnaseq_oral_cancer_metastasis_bench1.csv", 
                     stringsAsFactors=FALSE)
times1 <- round(bench1$time*10^(-9), 2)
times2 <- round(bench2$time*10^(-9), 2)
@
  On the colorectal cancer example in MD Section \ref{md-sec:colorectal}
  we achieved the following times:
  \Sexpr{times1[c(1:3)]} seconds, with $\alpha \in \{ 0.05, 0.5, 0.95 \}$, 
  respectively. \texttt{GRridge} and \texttt{SGL} with 
  $\alpha \in \{ 0.05, 0.5, 0.95 \}$ are 
  slightly faster with \Sexpr{times1[4]} and \Sexpr{times1[c(5:7)]} seconds, 
  respectively.
  \texttt{cMCP} (\Sexpr{times1[c(8:10)]} seconds) and \texttt{gel} 
  (\Sexpr{times1[c(11:13)]} seconds) are faster than \texttt{gren}. 
  The results for the oral cancer example in MD Section \ref{md-sec:oral} 
  are similar. \texttt{gren} 
  achieved 
