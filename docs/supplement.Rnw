...
\begin{document}
  ...
  \cite[]{neerincx_combination_2018} in Section \ref{sec:appcolon} . Finally, we
  introduce the ridge and lasso versions of the proposed method in Section
  ...
  \begin{align*}
  \E[\beta] &= g(\lambda_1,\lambda_2) \int_{-\infty}^{\infty} \beta \cdot \exp\[
  -\frac{1}{2} (\lambda_1 |\beta| + \lambda_2 \beta^2)\] d\beta=0, \\
...
The quality of feature selection is assessed by Cohen's kappa \cite[]{cohen_coefficient_1960}. This kappa is measures similarity between two sets of categorical values. One set contains the feature selection indicators: $s(j) := \mathbbm{1}\{\hat{\beta}_j \neq 0\}$, while the other set contains the true non-zero feature indicators: $t(j) := \mathbbm{1}\{\beta_j \neq 0\}$. We calculate Cohen's $\kappa$ as:
  ...
  In this section we present additional results to Section [! section ref here]
...
@
  ...
  \ref{fig:boxplots_micrornaseq_colorectal_cancer_res3_multipliers}. 
  From this figure we see that the estimates for \texttt{gren} are close to one.
  The estimates by \texttt{GRridge} show much more variation. We found no strong
...
<<boxplots_micrornaseq_colorectal_cancer_res3_multipliers, fig.cap="Results for the colorectal cancer example with 100 realisations of 3 random groups of features.", out.width="50%", fig.asp=1>>=
@

  We repeated the simulation with ten evenly sized groups. Figure 
  [! figure ref here] presents the estimated penalty multipliers for 100 random 
  ...
  then the other models. In addition, the median penalty multipliers for the 
  three \texttt{gren} models are all between [! medians here], while the 
  \texttt{GRridge} median estimates range from [! medians here]. This indicates 
  that \texttt{GRridge} suffers from a slight bias in the estimation, while the 
  \texttt{gren} estimates appear to be unbiased.
<<boxplots_micrornaseq_colorectal_cancer_res4_multipliers, fig.cap="Results for the colorectal cancer example with 100 realisations of 10 random groups of features, plotted (a) with outliers, and (b) without outliers.", out.width="100%", fig.asp=1/2>>=
@
  ...
  of all ${50}\choose{2}$ intersections of selected features and present the 
  results in Figure [! figure ref here].
  ...
  \section{Application to metabolomics in alzheimer}\label{sec:alzheimer}
	...
	To estimate performance measures AUC and BSS, we split the data into 120 
	training instances and 54 test instances. 
	The training data is used to fit the models, while the test data
	is used to estimate the performance measures on. We included the different 
	co-data types seperately and combined, which resulted in the estimated 
	multipliers and performance measures presented in Figure 
	\ref{fig:barplot_lines_metabolomics_alzheimer_res5_auc}.
<<barplot_lines_metabolomics_alzheimer_res5_auc, fig.cap="Results for the alzheimer example with (a) the estimated penalty multipliers for the quality score, (b) the estimated penalty multipliers for the network degree, (c) the estimated AUC, and (d) the estimated Brier skill score.", out.width="100%", fig.asp=1>>=
@
  ...
  \section{Application to microRNAs in cervical cancer}
...
<<barplots_micrornaseq_cervical_cancer_res1_auc, fig.cap="Estimated penalty multipliers for conservation status co-data", out.width="50%", fig.asp=2/3>>=
@

<<lines_micrornaseq_cervical_cancer_res1_auc, fig.cap="Cross-validated AUC in cervicl cancer example", out.width="50%", fig.asp=1>>=
@
