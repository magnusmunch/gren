...
\begin{document}
  ...
  (two $\alpha$'s for \texttt{gren}) that gave the best
  ...
  Additionally, the metabolites are highly collinear, which further impairs 
  estimation. The group lasso extensions depend on just one penalty parameter,
  so suffer less from the small number of features.
