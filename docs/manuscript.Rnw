...
\begin{document}
	...
	\begin{abstract}
		...
		applications in two different cancer studies show
	...
	\ref{sec:oral}. The first study is concerned with treatment 
	response prediction for colorectal cancer patients, using sequenced
	microRNAs. In the second study we predict lymph node metastases in oral cancer
	patients, using RNAseq data. In both settings additional information on the 
	features is available in the form of a partitioning. Furthermore, in both
	studies data is sparse, so any extra included information may help to boost
	predictive performance. Inclusion of such extra information in 
	...
	the group lasso
	\cite[]{meier_group_2008, yuan_model_2006}, because
	the group lasso penalises and selects features at the group level. 
	There are two limitations to this approach: the group lasso (a) selects entire
	groups instead of single 
	features and (b) does not penalise adaptively: all groups are penalised 
	equally. Extensions such as the sparse group lasso 
	\cite[]{simon_sparse-group_2013} deal with (a), but do not address (b).
	...
	with differential penalization is the estimation of the penalty 
	...
	In line with the above, our proposed method is (a) objective: the 
	hyper-parameters are estimated from the data and not chosen on subjective 
	...
	user: only the data and grouping of the features is required. We emphasize 
	that our method is different from recent developments in grouped penalized 
	regression such as \cite{dondelinger_joint_2018}, where instead of the 
	features, the observations are partitioned.
	
	Recently, \cite{zhang_novel_2019} introduced Variational Bayes Variable
	Selection (VBVS), a method similar to ours in
	spirit. Nevertheless, one clear advantage of our method over VBVS is the
	possibility of adaptive inclusion of external information on the features. 
	Bayesian versions of support vector 
	machines have been used in classification problems as well 
	\cite[]{chakraborty_bayesian_2011}, but these methods lack 
	...
	\ref{sec:colorectal} and \ref{sec:oral}, we demonstrate the approach in 
  ...
  Throughout the following we assume that the geometric mean of the multipliers, 
	...
	\begin{align}
	  p_{\hat{\blambda}}(\bbeta | \y) & =
	  \frac{\int_{\btau} \L_{\hat{\blambda}}(\y, 
	  \bbeta, \btau) \, d\btau}{p_{\hat{\blambda}}(\y)} = 
	  ...
	  \L_{\blambda}(\y, \bbeta, \btau) \, d\bbeta d\btau. \label{eq:mmlestimate}
	\end{align}
	...
	For the regular elastic net, \texttt{gren}, \texttt{cMCP}, 
	...
	\begin{enumerate}[(i)]
		\item \label{it:scenario1} differential signal between the groups and uniformly
		distributed model
		parameters,
		\item \label{it:scenario2} a large number of small groups of features,
		\item \label{it:scenario3} no differential signal between the groups, but 
		strong correlations 
		within groups of features,
		\item \label{it:scenario4} differential signal between the groups and 
		heavy-tailed distributed 
		model parameters, and
		\item \label{it:scenario5}a very sparse setting with no signal in some of 
		the groups.
	\end{enumerate}
	Full descriptions of the scenarios are given in Section 
	\ref{sm-sec:simulations} of 
	the SM. In Scenario (\ref{it:scenario1}), the distribution of model 
	parameters is far
	from the assumed ridge/elastic net/lasso distributions, so we expect
	all methods to underperform. In Scenario (\ref{it:scenario2}), 
	which is similar to
	...
	estimate. In Scenario (\ref{it:scenario3}), we expect all methods to 
	underperform due to the high correlations between the features. The
	Scenario (\ref{it:scenario4}) is expected to yield a better \texttt{gren}
	performance, since
	\texttt{gren} assumes a heavier-tailed distribution and is designed to 
	detect differential signal between
	the groups. Scenario (\ref{it:scenario5}) is expected to yield a bad 
	performance for all methods, because of the sparse truth.
  ...
  Scenarios. We 
	...
	Section \ref{sm-sec:simulations} of the SM.
	
	Overall, the ridge methods outperform the elastic net methods, which in turn
	outperform the group lasso methods, in terms of Brier skill score.
	In Scenario (\ref{it:scenario1})
	\texttt{gren} and \texttt{GRridge} outperform the other methods in terms
	of AUC.
	Scenario (\ref{it:scenario2}) the AUC follows our expectation: \texttt{gren}, 
	and to
	a lesser extent \texttt{GRridge} underperform due to overfitting. Suprisingly,
	the regular elastic net outperforms the group lasso methods.
	In Scenario (\ref{it:scenario3}), \texttt{gren} and to a lesses extent the
	...
	impair penalty parameter estimation. Scenario (\ref{it:scenario4}) follows the 
	...
	Surprisingly, in Scenario (\ref{it:scenario5}), \texttt{gren} outperforms all
	other methods, which is an indication that \texttt{gren} is able to pick up 
	the sparse differential signal.
	
<<lines_simulations_res1_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (\\ref{it:scenario1}).", out.width="100%", fig.asp=1/2>>=
@

<<lines_simulations_res2_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (\\ref{it:scenario2}).", out.width="100%", fig.asp=1/2>>=
@

<<lines_simulations_res3_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (\\ref{it:scenario3}).", out.width="100%", fig.asp=1/2>>=
@

<<lines_simulations_res4_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (\\ref{it:scenario4}).", out.width="100%", fig.asp=1/2>>=
@

<<lines_simulations_res5_auc_briers, fig.cap="Average (a) AUC and (b) Brier skill score in simulation Scenario (\\ref{it:scenario5}).", out.width="100%", fig.asp=1/2>>=
@
  ...
  \ref{sm-sec:colorectal}. For the methods with a
  tuning parameter $\alpha$, we show the best performing $\alpha$ here and refer
  the reader to SM Section \ref{sm-sec:colorectal} for the remaining $\alpha$'s.
  ...
  scores are presented in SM Section \ref{sm-sec:colorectal}. 
  In addition, we investigated the sensitivity of
  the multiplier estimation in SM Section 
  \ref{sm-sec:colorectal}. Cross-validation of the global $\lambda$ in 
  \texttt{GRridge} resulted in numerical issues. We therefore estimated this
  global parameter on the full data for \texttt{GRridge}.
  
  The estimated penalty multipliers are according to expectation: the highly
  expressed group receives the lowest penalty, followed by the medium expressed
  group and the non-expressed group. 
  \texttt{gren} seems to outperform the other methods for a range of smaller 
  model sizes. Selection of larger models is impaired by the large
  $\alpha$, a property inherent to $L_1$-norm penalization. With a smaller
  $\alpha$ larger models are possible (see SM Section \ref{sm-sec:colorectal}). 
  ...
  albeit only slightly for \texttt{GRridge}.
...
@
  ...
  \section{Application to RNAseq in oral cancer}\label{sec:oral}
  ...
  We estimated AUC on an independent validation set of 97 
  patients \cite[]{mes_prognostic_2017}, containing features with
  microarray expression levels, normalised to account
  for a scale difference with the RNAseq training data. We present estimated
  AUC on this validation set for \texttt{GRridge}, ridge, \texttt{gren}, 
  ...
  \ref{fig:barplot_lines_rnaseq_oral_cancer_res1_auc}. For the methods with
  an $\alpha$ parameter, we pick the best performing one.
  Results for \texttt{SGL}, \texttt{gel}, other values of $\alpha$, and the 
  Brier skill scores are presented in SM Section \ref{sm-sec:oral}.
...
<<barplot_lines_rnaseq_oral_cancer_res1_auc, fig.cap="Estimated (a) penalty multipliers for the cis-correlations and (b) external $p$-values, and (c) estimated AUC in the oral cancer example.", out.width="100%", fig.asp=1>>=
@
  
  \section{Additional applications}\label{sec:applications}
  Two additional applications are presented in SM Sections 
  \ref{sm-sec:alzheimer} and \ref{sm-sec:cervical}. The first one is concerned
  with diagnosis of 
  ...
  example, \texttt{gren} performs worse than the group lasso methods. This is 
  ...
  model sizes.
	...
	Section \ref{sec:simulations} and the applications in Sections 
	\ref{sec:colorectal}-\ref{sec:oral}, and SM Sections \ref{sm-sec:colorectal}-
	\ref{sm-sec:cervical} adaptation to 
	external data may be beneficial for prediction and feature selection. 
	We show that to be true for both prognostic and diagnostic targets,
	as well as a range of marker types (RNAseqs, microRNAs, and metabolites).
	...
	\texttt{gren} fits a reweigthed elastic net. The group lasso uses a 
	qualitatively different penalty with one overall penalty parameter; 
	...
	the original data and calculated the intersections sizes of selected
	features. Compared to the regular elastic net, the inclusion of extra 
	information
	increased the selection stability (see SM Section \ref{sm-sec:colorectal}). 
	In addition, we investigated the performance of penalty multiplier estimation 
	by creating 100 random partitionings of the features
  (see SM Section \ref{sm-sec:colorectal}). We found that the estimated penalty 
  parameters tend to cluster around one, as expected with a random grouping.

	A possible weak point of the proposed method is the double EM loop, which has
	positive probability of ending up in a local optimum. In the applications
	discussed in Sections \ref{sec:colorectal}-
	\ref{sec:oral}, and SM Sections \ref{sm-sec:colorectal}-
	...
	values. In our 
	experience, reasonable starting values are obtained from an initial 
	\texttt{GRridge} fit. Alternatively, a skeptical user may set 
...
bench2 <- read.table("results/rnaseq_oral_cancer_bench1.csv", 
                     stringsAsFactors=FALSE)
...
@
  ...
  implementation we have reduced computational time considerably by applying
  some computational tricks (see SM Section 
  \ref{sm-sec:computationalcomplexity}) and implementing computationally heavy
  ...
  \ref{sec:oral}. All timings were obtained on a Macbook 
  ...
  The results for the oral cancer example are similar. \texttt{gren} 
  achieved 
