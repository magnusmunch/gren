...
\begin{document}
	...
	Examples of such a study with extra information available are investigated in
  ...
  the reader to SM Section [! section ref here] for more comparisons to 
  different $\alpha$s and to the sparse
  group lasso (\texttt{SGL}) by \cite{simon_sparse-group_2013} and the group
  exponential lasso (\texttt{gel}) \cite[]{breheny_group_2015}.
  ...
  performance measures comparisons are presented in Section [! section ref here]
  of the SM. In addition, we investigated the sensitivity of
  the multiplier estimation using this data set in SM Section 
  [! section ref here]. A small sidenote: We experienced numerical issues with 
  ...
  outperform their respective non-group-regularized versions, albeit it only
  slightly for \texttt{GRridge}.
...
@
  ...
  these patients, 76 suffered from lymph node metastasis, while 57 did not. 
  For a thorough introduction of these data, see \cite{te_beest_improved_2017}.
  
  As co-data we considered two sources: (a) the cis-correlation between the
  the RNASeqv2 data and DNA copy numbers on the same patients, 
  quantified by Kendall's $\tau$ and binned into five groups, and (b) 
  $p$-values from an external microarray data set described in 
  ...
  \ref{sec:appcolon}. However, the co-data consists of two partitions with 
  overlapping groups, so we use an extension of \texttt{cMCP} that is able to 
  handle multiple groupings \cite[]{zeng_overlapping_2016}. We split the data 
  into 120 training instances and 97 test instances, trained
  the models on the training set and estimated AUC on the test set. We present 
  ...
  The estimated penalty multipliers follow the expected pattern. Larger 
  cis-correlations receive smaller penalties. Likewise, smaller $p$-values are
  also penalized less. This pattern is slightly less pronounced in \texttt{gren}
  with $\alpha=0.95$ than in the other methods. The predictive performance of
  ...
  where we use as grouping the cross-tabulation of the two paritions. In this 
  example, ridge and \texttt{GRridge} perform almost identical.
<<barplot_lines_rnaseq_lymph_node_metastasis_res1_auc, fig.cap="Results for the lymph node metastasis example with (a) the estimated penalty multipliers for the cis-correlations, (b) the estimated penalty multipliers for the p-values, and (c) the estimated AUC. OcMCP refers to the overlapping group cMCP.", out.width="100%", fig.asp=1>>=
@
	...
	\ref{sec:simulations} and the application examples with different types of 
	outcome data (prognostic and diagnostic) and marker types (RNAseq, microRNAs, 
	and metabolomics) in Sections [! section refs here], and the SM
	Sections [! section refs here], adaptation to 
	...
	\url{https://CRAN.R-project.org/package=gren}. All materials needed to   
	...
	\url{http://pub.math.leidenuniv.nl/~munchmm/}.
