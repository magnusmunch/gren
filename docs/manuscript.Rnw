...
\begin{document}
  ...
  We conclude with a discussion of some of the benefits and drawbacks of the
  proposed \texttt{gren}.
	...
	$\lambda_1 > 0$ and $\lambda_2 > 0$ are the `global' penalty 
  ...
  weighted by their 
	...
	(\lambda'_g)^{|\G(g)|}=1$, with $|\G(g)|$ the number of features in group $g$. 
	The multipliers appear in square root form in the
	...
	well-handled. Addition of the penalty terms also prevents quasi-complete
	separation in logistic regression, a common phenomenon in small $n$ studies.
  ...
  (ii) classes of metabolites (see SM Section \ref{sm-sec:alzheimer}),
  ...
  (v) quality scores of the features (see SM Section \ref{sm-sec:alzheimer}), 
  and (vii) the node degrees of a network estimated on the feature 
  data (see SM Section \ref{sm-sec:alzheimer}).
  ...
  prediction.
	...
	\ref{sm-sec:objectivefunction} for the 
	...
	\ref{sm-sec:polyagamma} for more 
	...
	update them by (\ref{sm-eq:VBupdateequations}) in SM Section 
	\ref{sm-sec:updatingequations} until convergence. 
	Naive calculation of the variational
	parameters is computationally expensive. In SM Section 
	\ref{sm-sec:computationalcomplexity} we show that 
	...
	\ref{sm-sec:updatingequations} for the full 
	derivation and the $d^{(k)}_g$ terms). An estimate
	of the new penalty multipliers is now given by 
	(\ref{sm-eq:mmlupdateequation}) in SM Section \ref{sm-sec:updatingequations}. 
	Although the solution to
	(\ref{sm-eq:mmlupdateequation}) in the SM is not available in
    ...
    \label{alg:summaryalg}
	  \begin{algorithmic}[h!]
		  ...
		  for $j=1, \dots, p$ using (\ref{sm-eq:VBupdateequations}) in the SM}
		  \EndWhile 
		  \State{Update $\blambda'$ by (\ref{sm-eq:mmlupdateequation}) in the SM}
		  \EndWhile
	  \end{algorithmic}
  \end{algorithm}
  ...
  \section{Simulations and applications}\label{sec:simandapp}
	...
	\ref{sm-sec:simulations}. Here, we summarise the results.
	...
	In addition to simulations from the correct model, i.e., model 
	(\ref{eq:enmodel2}), we simulated from an incorrect model to investigate 
	the performance of \texttt{gren} under model misspecification. We investigated
	both link misspecification and non-linear feature effects misspecification. 
	The details of the simulations and detailed results are given in SM Section 
	\ref{sm-sec:modelmisspecification}. In all investigated Scenarios 
	\texttt{gren} outperformed the regular elastic net in terms of predictive 
	performance; an indication that \texttt{gren} is somewhat robust to model
	misspecification.
...
@
  ...
  features. 
	...
	information
	increases the selection stability (see SM Section \ref{sm-sec:colorectal}). 
  ...
  \ref{sm-sec:computationalcomplexity}) and implementing some
  ...
  results in SM Section \ref{sm-sec:timings}. In general we found that 
