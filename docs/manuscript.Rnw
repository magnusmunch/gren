...
\begin{document}
  ...
  Throughout the following we assume that the geometric mean of the multipliers, 
	...
	(see Supplementary Material (SM) Section [! section reference here] for 
	...
	automatic feature selection, we estimate 
	...
	\end{enumerate}
	Full descriptions of the scenarios are given in Section [! section ref here] 
	of 
	...
	all methods to underperform. In the second scenario, we expect \texttt{gren}
	and \texttt{GRridge} to overfit due to the large number of hyperparameters to 
	estimate, while we expect the other methods to perform better, due to a
	lack of overfitting. In scenario three, we expect all methods to underperform
	due to the high correlations between the features. The fourth 
	scenario is conjectured to yield a better \texttt{gren} performance, since
	\texttt{gren} assumes a heavier-tailed distribution and is designed to 
	incorporate a differential signal between
	...
	$p=1000$. We varied the covariance matrix of the features and simulation of 
	the model parameters $\beta_j$, $j=1, \dots, p$ between the
	scenarios (see SM Section [! section ref here] for the exact 
	parametrisations). We 
  ...
  the reader to SM Section [! section ref here] for comparisons with 
  different $\alpha$'s.
  ...
  (SM [! section ref here]). Overall, it seems as though the inclusion of the
...
@
	...
	Although our method can be considered weakly adaptive and is essentially 
	a reweighted elastic net, it is different from 
	...
	increased the selection stability (see SM Section [! section ref here]). 
	In addition, we investigated 
	the variance and bias in penalty multiplier estimation by examining the
	distribution
  ...
  tend to cluster around one, as expected for an unbiased estimation method.

	As touched upon in Section \ref{sec:introduction}, another obvious 
...
@
  ...
  larger combined space of model- and hyper-parameters. 
	...
	\url{http://pub.math.leidenuniv.nl/~munchmm/}. All results may be recreated
	from \url{https://github.com/magnusmunch/gren}.
