...
@
  ...
  tend to cluster around one, as expected for an unbiased estimation method.
	...
	even superior to (extensions of) the group lasso.
...
@
  ...
  reliable EB step. This approach is often used in more 
  computational disciplines, where the problem is viewed as purely algorithmic 
  and the EB and VB iterations are interpreted as optimisation steps along the 
  gradient in the larger combined model- and hyper-parameters space. 
