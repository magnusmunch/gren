...
\begin{document}
  ...
  Throughout the following we assume that the geometric mean of the multipliers, 
	...
	In Scenario (\ref{it:scenario3}), we expect all methods to underperform
	due to the high correlations between the features. The
	...
	from $p=900$ to $p=1000$ over the Scenarios. We varied the 
  ...
  Scenarios (see SM Section \ref{sm-sec:simulations} for the exact 
	...
	For the methods with an $\alpha$ tuning parameter, we show the one that gives
	the best performance. Results for the other $\alpha$'s and additional
	performance measures are presented in Section 
	\ref{sm-sec:simulations} of the SM.
	
	In Scenario (\ref{it:scenario1})
	\texttt{gren} and \texttt{GRridge} don't suffer much from the wrong 
	distributional assumptions as they outperform the other methods.
	Scenario (\ref{it:scenario2}) follows our expectation \texttt{gren}, and to
	a lesser extent \texttt{GRridge} underperform due to overfitting. Suprisingly,
	the regular elastic net outperforms the group lasso methods, which are 
	designed to handle situations such as Scenario (\ref{it:scenario2}).
	In Scenario (\ref{it:scenario3}), \texttt{gren} and to a lesses extent the
	regular elastic net suffer more from the higher correlations. Here,
	the regular elastic net outperforms \texttt{gren}, which is an indication
	that the estimation of the penalty parameters is impaired by high
	correlation between features. Scenario (\ref{it:scenario4}) follows the 
	expected pattern, with the group-regularized methods (\texttt{GRridge} and
	...
	very sparse differential signals between the groups.
...
<<lines_simulations_res2_auc_briers, fig.cap="Estimated (a) AUC and (b) Brier skill score in simulation setting 2.", out.width="100%", fig.asp=1/2>>=
@
...
@
  ...
  another extension of the group lasso, the sparse
  group lasso by \cite{simon_sparse-group_2013}
  (\texttt{SGL}), and \texttt{gel} are presented in the SM Section 
  \ref{sm-sec:colorectal}. For the methods with a
  tuning parameter $\alpha$, we picked the $\alpha$ 
  that gave the best performance and refer
  ...
  model sizes in terms of AUC 
  (Figure \ref{fig:barplot_lines_micrornaseq_colorectal_cancer_res1_auc}b). 
  Selection of larger models is difficult with large values of $\alpha$, an 
  inherent property of the strong $L_1$-norm penalization. With lower values of
  $\alpha$, larger models are possible (see SM Section \ref{sm-sec:colorectal})
  for comparisons with smaller values of $\alpha$). The performance of
  \texttt{cMCP} is somewhat unstable. This unstable estimation is an issue for   
  all group lasso extensions that we investigated.
  (SM Section \ref{sm-sec:colorectal}). Overall, the 
...
@
  ...
  The aim of our second example study is to predict lymph node metastasis in
  ...
  3096 transformed and normalised TCGA RNASeqv2 expression values of 3096 genes 
  ...
  \ref{fig:barplot_lines_rnaseq_oral_cancer_res1_auc}. Additional
  ...
  (Figure \ref{fig:barplot_lines_rnaseq_oral_cancer_res1_auc}a): 
...
@
  ...
  Two more applications are presented in SM Sections \ref{sm-sec:alzheimer} and
  \ref{sm-sec:cervical}. The first one is concerned with diagnosis of 
  ...
  further impairs estimation. The group lasso extensions depend on just one
  penalty parameter, so suffer less from the small number of features and
  negative correlations.
  
  In the second extra example, we attempt to diagnose 56 women with cervical 
  lesions using 2576 sequenced microRNAs with the purpose of 
  selecting relevent markers for cervical cancer screening programmes. We use 
  include one partitioning of the features to enhance predictive performance and 
  ...
  model sizes.
	...
	as well as a range of marker types (RNAseq, microRNAs, or metabolomics).
...
@
  The double EM loop runs the danger of excessive computation time. In our 
  implementation we have reduced computational time considerably by applying
  some computational tricks (see SM Section 
  \ref{sm-sec:computationalcomplexity}). In addition, the computationally heavy
  parts were implemented in \texttt{C++} to further reduce the computation time. 
  To assess this reduction, we have compared computation times to the
