...
\begin{document}
	...
	An example of such a study with extra information available is investigated in
	Sections [! section refs here]. The first one is concerned with treatment 
	response prediction for colorectal cancer patients, based on sequenced
	microRNAs. In the second example we attempt to predict lymph node metastases,
	using RNAseq data. In both settings we have additional information on the 
	features in the form of a partitioning available. Furthermore, in both
	examples data is sparse, so we would like to 
	include the additional information to enhance predictive performance of our
	models. Inclusion of the partitions is not straightforward using standard 
	methods available in literature. At first sight, such
	an \textit{a priori} grouping of the features suggests the group lasso
	\cite[]{meier_group_2008, yuan_model_2006} (or one of its extensions), which
	penalises and selects features at the group level. There are two limitations 
	to the group lasso: (a) it selects groups as a whole instead of single 
	features and (b) it is not adaptive: the amount of penalization a group
	receives depends on a `global' penalty parameter common to all groups.
	Extensions such as the sparse group lasso \cite[]{simon_sparse-group_2013}
	partly deal with (a), but do not address (b).
	...
	aid in prediction and feature selection. We would like to emphasize that our
	method is different from recent developments in grouped penalized regression
	such as \cite{dondelinger_joint_nodate}, where the grouping is over the
	observations as opposed to the features 

	In line with the above, our proposed method is (a) subjective: the few
	remaining tuning parameters may be cross-validated, (b) adaptive: prior
	information is automatically and subjectively weighed with respect to its
	informativeness, (c) fast compared to full Bayesian analysis or 
	cross-validation, and (d) requires minimal input by the user: only the data 
	and grouping of the features is required.
	...
	\ref{sec:simulations} and Sections [! section ref here] and 
	[! section ref here], we demonstrate the approach in a
	simulation study and a two real data examples, respectively. In Section 
	\ref{sec:discussion} we conclude by both discussing some differences and
	...
	In elastic net regression, the penalised likelihood is maximised to yield
	parameter estimates:
	$$
	\hat{\bbeta} := \underset{\bbeta}{\argmax} \ell(\y ; \bbeta) - 
	\frac{\lambda_1}{2} \norm{\bbeta}_1 - \frac{\lambda_2}{2} \norm{\bbeta}_2^2,
	$$
	with $\lambda_1, \lambda_2 \in \mathbb{R}_{>0}$ and $\y = \begin{bmatrix} 
	y_1 & \cdots & y_n \end{bmatrix} \tr$. The likelihood term is thus 
	complemented with a penalty term consisting of the $L_1$ and $L_2$-norm of the
	parameters, multiplied by respective penalty parameters $\lambda_1$ and
	$\lambda_2$. We have scaled the penalty parameters by $\frac{1}{2}$, to make
	them correspond with the Bayesian version in the following. The maximiser of
	the penalised likelihood corresponds to the posterior mode of a Bayesian
	elastic model with prior \cite[]{li_bayesian_2010,zou_regularization_2005}:
	\begin{equation}\label{eq:elasticnetprior}
	  \beta_j \sim g(\lambda_1,\lambda_2) \cdot \exp\[-\frac{1}{2} (\lambda_1
	  |\beta_j| + \lambda_2 \beta_j^2)\].
	\end{equation}
	Here, $g(\lambda_1,\lambda_2)$ denotes a normalizing constant that is 
	dependent on $\lambda_1$ and $\lambda_2$ (given in Section 2 of the 
	Supplementary Material (SM), along with further details on the elastic net
	prior). The elastic net combines $L_1$ and $L_2$-norm penalisation such that
	the model parameters are shrunken towards zero. The $L_1$-norm may set some of
	the estimates exactly to zero, thus automatically selecting features. The 
	$L_2$-norm ensures that correlated features behave similarly, as one would
	generally require. We extend the elastic net model to the generalised elastic
	net to allow for different penalty parameters per group of features.
	...
	$g$ for $g=1, \dots, G$. To apply differential penalisation we include a 
	group-specific weight $w_g \in \mathbb{R}_{>0}$. The model parameters are now
	estimated by:
	\begin{align}
	  \hat{\bbeta} & := \underset{\bbeta}{\argmax} \ell(\y ; \bbeta) - 
	  ...
	  & = \underset{\bbeta}{\argmax} \ell(\y ; \bbeta) -  \frac{\lambda_1}{2} 
	  \sum_{g=1}^G \sqrt{\lambda'_g} \sum_{j \in \G(g)} | \beta_j| - 
	  \frac{\lambda_2}{2} \sum_{g=1}^G \lambda'_g \sum_{j \in \G(g)} \beta_j^2, 
	  \nonumber
	\end{align}
	where we wrote $w^2_g = \lambda'_g$ to emphasise that these group-specific
	weights may be interpreted as penalty multipliers. Throughout the following we
	assume that the geometric mean of the multipliers, weighted by their 
	respective group sizes, is one, such that the average shrinkage of the model
	parameters is determined by the `global' $\lambda_1$ and $\lambda_2$. That is,
	...
	\cite{li_bayesian_2010} show that (\ref{eq:elasticnetprior}) may be written as
	...
	on the marginal likelihood of this model (see Section 3 of the SM for the 
	...
	choices of $\alpha$. We recommend the latter. Otherwise we note that, in our 
	experience, the choice $\alpha=0.5$ often gives good results.

	For estimating the penalty multipliers, the intractable posterior expectation
	in (\ref{eq:ebEM}) is approximated by the variational posterior:
	$$
	...
	and omics data problems are no exception. A selection of biomarkers may lead 
  ...
  The presented data is from a deep sequencing analysis on microRNAs 
  \cite[]{neerincx_combination_2018}. The study was done in 88 treated 
  colorectal cancer patients, with the aim of predicting treatment response,
  coded as either non-progressive/remission (70 patients) or progressive 
  (18 patients). After pre-processing and normalisation, 2114 microRNAs 
  remained. In addition to the 2114 microRNAs, four clinical covariates were
  included in the analysis: prior use of adjuvant therapy (binary),
  the type of systemic treatment regimen (ternary), age, and primary tumor
  differentiation (binary). These clinical covariates are included as 
  unpenalized features in the model.
  ...
  primary colorectal tumour tissue \cite[]{neerincx_mir_2015}. This yielded 221
  microRNAs that were differentially expressed in both comparisons ($\text{FDR}
  \leq 0.05$), versus 1893 not differentially expressed microRNAs. We expect
  that incorporation of this partitioning enhances therapy response 
  classification, because tumor-specific miRNAs might be more relevant than 
  non-specific ones. In addition, we divided the differentially expressed
  microRNAs even further into 127 highly differentially expressed microRNAs
  ($\text{FDR} \leq 0.001$) and 94 medium differentially expressed microRNAs 
  ($0.001 < \text{FDR} \leq 0.05$).  We will refer to this second partitioning
  as the three-group setting, as opposed to the first, the two-group setting.

  We split the data into 61 training, and 27 test instances and
  compared the performance of \texttt{gren} on the test set to ridge, 
  \texttt{GRridge}, elastic net, and the composite minimax
  concave penalty (\texttt{cMCP}) by \cite{breheny_penalized_2009}. Further
  comparisons to the sparse
  group lasso (\texttt{SGL}) by \cite{simon_sparse-group_2013} and the group
  exponential lasso (\texttt{gel}) \cite[]{breheny_group_2015} are presented in 
  the SM [! section ref here]. We compared the methods on AUC. More performance
  measures are presented in the SM [! section ref here].
  For the elastic net methods (including \texttt{gren}) and \texttt{cMCP}, we 
  set $\alpha \in \{ 0.05, 0.5, 0.95\}$. We experienced numerical issues with 
  ...
  \ref{fig:barplot_lines_micrornaseq_colorectal_cancer_res2_auc}. 
  ...
  sizes. This is especially pronounced for the larger models. The performance of
  \texttt{cmcp} is somewhat unstable. We noticed this is in general
  for the group-lasso and its extensions (see SM [! section ref here]). Overall,
  it seems as though the inclusion of the extra information on the features 
  benefited predictive performance: both \texttt{GRridge} and \texttt{gren} 
  outperform their respective non-group-regularized versions.
...
@
  ...
  \section{Application to RNAseq in lymph node cancer}\label{sec:applymph}
  In this application, the aim is to predict lymph node metastasis in patients 
  ...
  normalised TCGA RNASeqv2 profiles of head-and-neck squamous cell 
  carcinomas on 133 patients with HPV-negative tumours in the oral cavity. Of 
  these patients, 76 had lymph node metastasis, and 57 did not. For a thorough 
  introduction of these data, see \cite{te_beest_improved_2017}.
  
  As co-data we considered two sources: (a) the cis-correlation between TCGA DNA
  copy number measurements and the RNASeqv2 data on the same patients, 
  quantified through Kendall's $\tau$ and binned into five groups, and (b) 
  p-values from an external microarray data set \cite[]{mes_prognostic_2017}, 
  binned into five groups. We expect features 
  with a positive Kendall's $\tau$ to be more important in metastasis prediction   
  \cite[]{masayesva_gene_2004}. Likewise, we expect features with a low 
  p-value to be more important.
  
  We split our data into 120 training instances and 97 test instances, trained
  the models on the training set and estimated AUC on the test set. Other 
  performance measures are presented in the SM Section [! section ref here]. We
  compared the same models as in Section \ref{sec:appcolon}. In this 
  application, the co-data consists of two partitions with overlapping groups.
  \texttt{cMCP} is not designed to deal with overlapping groups, so we 
  cross-tabulate the two co-data partitions to create one paritioning with 25 
  groups for \texttt{cMCP}. In addition, we compare to an extension of 
  \texttt{cMCP} that is able to handle multiple groupings 
  \cite[]{zeng_overlapping_2016}. We present the results in Figure 
  \ref{fig:barplot_lines_rnaseq_lymph_node_metastasis_res1_auc}. 
  Additional comparisons to \texttt{SGL}, \texttt{gel} and 
  the overlapping group version of \texttt{gel} \cite[]{zeng_overlapping_2016}
  \cite[]{breheny_group_2015} are found in the SM Section [! section ref here].
...
@
  ...
  We present additional applications and comparisons in the SM Sections 
  [! sections ref here].
	...
	adaptation more robust. As can be seen from the simulations and real data 
	example in Sections \ref{sec:simulations} and [! section ref here], 
	adaptation to 
	external data may be beneficial for prediction and feature selection. We 
	believe that this is due to the `borrowing of information' effect: estimates 
	that are believed to be similar are shrunken in the same way, yielding 
	overall, better estimates.

	Another obvious comparison is to the group lasso \cite[]{meier_group_2008,
	yuan_model_2006}. Although it is similar in the sense that it shrinks on the 
	group level, the group lasso is built upon an entirely different philosophy: 
	it sets whole groups of features to zero. The intended application of such a 
	group-wise penalty is to small interpretable groups of features, like, for 
	example, dummies of a categorical variable. Another difference between 
	...
	as starting values for \texttt{gren}. In addition, we note that even a local
	optimum might give `good enough' predictions in many practical settings.
