...
@
	...
	\begin{subequations}\label{eq:enmodel2}
		...
		\end{align}
	\end{subequations}
	Here, $\mathcal{TG} \( k,\theta,\( x_l,x_u\) \)$ denotes the truncated gamma distribution with shape $k$, scale $\theta$, and domain $\(x_l,x_u\)$. In this Bayesian formulation the penalty parameters $\blambda = \begin{bmatrix} \lambda_1 & \lambda_2 & \lambda'_1 & \cdots & \lambda'_G \end{bmatrix} \tr$ play the role of the hyperparameters in a Bayesian hierarchical model.
