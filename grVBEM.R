...
# sum2 <- rev(cumsum(rev(sizes)))
grmagn_mono1 <- function(par, lambda2, sizes, sum1, sum2) {
  s <- pat[1]
  loglambdag <- par[-1]
  
  partsum <- sum((0.5*lambda2*rev(cumsum(rev(sum1*exp(cumsum(loglambdag))))) - 
                    (s - 0.5)*sum2*loglambdag)^2)
  constrsum <- sum(sum2*loglambdag)^2
  magn <- sqrt(partsum + constrsum)
  return(magn)
}
