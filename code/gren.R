...
gren <- function(x, y, m, unpenalized=NULL, partitions=NULL, alpha=0.5, 
  ...
  if(is.null(lambda)) {
    ...
    } else if(length(foldid)!=n | !all(foldid %in% c(1:n))) {
  ...
  # checks and auxiliary variables for the partitions
  if(lapply(partitions, length)!=r) {
  ...
  startparam <- est_param3(xr, xu, kappa, m, n, p, pred.start, phi, rep(phi, r), 
                           lambda2, lambdamultvecold, lambdamultvecold, 
                           intercept, unpenalized, FALSE, FALSE, TRUE)
  ...
  while(!conv & (iter1 < control$maxit)) {
    ...
    conv2 <- FALSE
    iter2 <- 0
    while(!conv2 & (iter2 < maxit.vb)) {
      iter2 <- iter2 + 1
      
      # estimating new model parameters
      newparam <- est_param3(xr, xu, kappa, m, n, p, ciold, phi, chiold, 
  ...
  if(posterior) {
    newparam <- est_param3(xr, xu, kappa, m, n, p, ci, phi, chi, lambda2, 
  ...
  # output list
  out <- list(alpha=alpha, lambdag=lambdagseq, 
              lambdag.est=lambdagseq[, lambda.iter + 1],
              vb.post=vb.post, freq.model=freq, iter=iter, conv=conv,
              args=argum)
  return(out)
