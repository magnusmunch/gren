...
for(k in 1:K) {
  ...
  if(any(which.const1)) {
    ...
    boot.conservation <- sort(as.numeric(mirsData$conservation))[!which.const1]
  } else {
    boot.conservation <- sort(as.numeric(mirsData$conservation))
  }
...
save(results2, file=paste(path.res, "gren_mirna_cervical_res2.Rdata", sep=""))
